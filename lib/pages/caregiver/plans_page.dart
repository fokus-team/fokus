        ...
        body: Column(
            ...
            children: <Widget>[
              AppHeader.normal(
                  ...
                  headerActionButtons: [
                    HeaderActionButton.normal(Icons.add,
                        '$_pageKey.header.addPlan', () => {log("Dodaj plan")}),
                    HeaderActionButton.normal(
                        Icons.calendar_today,
                        '$_pageKey.header.calendar',
                        () => {log("Kalendarz")},
                        Colors.amber)
                  ]),
...
Segment _getPlansSegment(
    {List<UIPlan> plans,
    String title,
    String noElementsMessage,
    Widget noElementsAction,
    context}) {
  return Segment(
    title: title,
          ...
          chips: <Widget>[
            AttributeChip.withIcon(
                content: AppLocales.of(context).translate(
                    '$_pageKey.content.tasks', {'NUM_TASKS': plan.taskCount}),
                color: Colors.indigo,
                icon: Icons.layers)
          ],
        ),
